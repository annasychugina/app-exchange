...
describe('getRates API', () => {
  it('request with correct url and params', async () => {
    ...
    expect(spyGetSuccess)
      .toHaveBeenLastCalledWith( "https://api.exchangeratesapi.io/latest", {"params": {"base": "RUB"}});

    await getRates('USD');
    expect(spyGetSuccess).toHaveBeenCalledTimes(2);
    expect(spyGetSuccess)
      .toHaveBeenLastCalledWith( "https://api.exchangeratesapi.io/latest", {"params": {"base": "USD"}});
