...
it('calls onCurrencyValueChange first ExchangeBlock', () => {
  ...
  rows.at(0).props().onCurrencyValueChange('34');
...
it('calls onCurrencyValueChange first ExchangeBlock', () => {
  ...
  blocks.at(1).props().onCurrencyValueChange('34');
  expect(onCurrencyValueChange).toBeCalledTimes(1);
  expect(onCurrencyValueChange).toHaveBeenLastCalledWith('34');
});
...
it('calls handleExchange when click button', () => {
  ...
  const wrapper = shallow(<ExchangeWidgetView {...mockProps}  isExchangeButtonDisabled={false} handleExchange={onExchange}/>);
...
it('calls onCurrencyChange first ExchangeBlock', () => {
  ...
  const blocks = wrapper.find(ExchangeBlock);
  blocks.at(0).props().onCurrencyChange('RUB');
...
it('calls onCurrencyChange second ExchangeBlock', () => {
  ...
  const blocks = wrapper.find(ExchangeBlock);
  blocks.at(1).props().onCurrencyChange('RUB');
